...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  nav {
    ...
    height: 64px; box-shadow: 0 2px 12px rgba(0,0,0,0.2);
  }
  ...
  .btn-submit {
    flex: 1; background: linear-gradient(135deg, var(--accent-dark), var(--accent));
    color: #fff; padding: 13px 32px; border-radius: 9px;
    font-size: 15px; font-weight: 700; border: none; cursor: pointer;
    transition: transform 0.2s, box-shadow 0.2s;
    box-shadow: 0 4px 14px rgba(0,0,0,0.18);
  }
  .btn-submit:hover { transform: translateY(-2px); box-shadow: 0 8px 22px rgba(0,0,0,0.22); }
  .btn-cancel {
    background: #fff; color: var(--accent); padding: 13px 24px;
    border-radius: 9px; font-size: 15px; font-weight: 600;
    text-decoration: none; border: 2px solid var(--accent-border);
    transition: background 0.2s, border-color 0.2s;
    display: inline-flex; align-items: center;
  }
  .btn-cancel:hover { background: var(--accent-light); border-color: var(--accent-text); }
</style>
</head>
...
<nav>
  ...
  <div class="nav-right">
    ...
    <a href="recruiter-jobs" class="btn-nav">My Jobs</a>
...
<div class="page-header">
  <div class="page-header-inner">
    <h1>&#9998; Edit Job</h1>
    <p>Update the job details below — changes will reflect immediately</p>
  </div>
</div>
  ...
  <div class="edit-card">
    ...
    <form action="editjob" method="post">
      ...
      <div class="form-group">
        ...
        <input type="text" name="title" value="${job.title}" required placeholder="e.g. Senior Java Developer">
      </div>

      <div class="form-group">
        <label>Description</label>
        <textarea name="description" required placeholder="Describe the role, responsibilities and requirements...">${job.description}</textarea>
      </div>

      <div class="form-row">
        <div class="form-group">
          <label>Location</label>
          <input type="text" name="location" value="${job.location}" required placeholder="e.g. Pune, Maharashtra">
        </div>
        <div class="form-group">
          <label>Salary (LPA)</label>
          <input type="number" name="salary" value="${job.salary}" required step="0.1" min="0" placeholder="e.g. 8.5">
        </div>
      </div>
