...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .alert-success { background: #dcfce7; border: 1px solid #86efac; border-left: 4px solid #16a34a; border-radius: 10px; padding: 14px 20px; color: #15803d; font-size: 14px; font-weight: 600; margin-bottom: 24px; display: flex; align-items: center; gap: 10px; }

  .profile-card { background: #fff; border-radius: 16px; box-shadow: 0 4px 24px rgba(0,0,0,0.08); border: 1px solid var(--accent-border); overflow: hidden; margin-bottom: 24px; }
  .profile-card-header { background: linear-gradient(135deg, var(--accent-light), #fff); padding: 32px 36px; display: flex; align-items: center; gap: 24px; border-bottom: 1px solid var(--accent-border); }
  .avatar { width: 80px; height: 80px; background: linear-gradient(135deg, var(--accent-dark), var(--accent)); border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 32px; font-weight: 900; color: #fff; flex-shrink: 0; box-shadow: 0 4px 14px rgba(0,0,0,0.2); }
  .profile-header-info h2 { font-size: 22px; font-weight: 800; color: #1e293b; margin-bottom: 4px; }
  .role-tag { display: inline-block; background: var(--accent); color: #fff; font-size: 11px; font-weight: 700; padding: 3px 12px; border-radius: 20px; text-transform: uppercase; letter-spacing: 0.8px; }

  .profile-fields { padding: 32px 36px; }
  .field-row { display: flex; align-items: flex-start; padding: 18px 0; border-bottom: 1px solid #f3f4f6; }
  .field-row:last-child { border-bottom: none; }
  .field-icon { width: 40px; height: 40px; background: var(--accent-light); border-radius: 10px; display: flex; align-items: center; justify-content: center; font-size: 17px; font-weight: 700; margin-right: 20px; flex-shrink: 0; color: var(--accent); }
  ...
  .profile-actions { padding: 0 36px 32px; display: flex; gap: 14px; align-items: center; }
  .btn-update { background: linear-gradient(135deg, var(--accent-dark), var(--accent)); color: #fff; padding: 13px 32px; border-radius: 9px; font-size: 15px; font-weight: 700; text-decoration: none; border: none; cursor: pointer; display: inline-flex; align-items: center; gap: 8px; transition: transform 0.2s, box-shadow 0.2s; box-shadow: 0 4px 14px rgba(0,0,0,0.18); }
  .btn-update:hover { transform: translateY(-2px); }
  .btn-back { background: #fff; color: var(--accent); padding: 13px 24px; border-radius: 9px; font-size: 15px; font-weight: 600; text-decoration: none; border: 2px solid var(--accent-border); transition: background 0.2s; }
...
<div class="main">
  ...
  <div class="profile-card">
    ...
    <div class="profile-fields">
      <div class="field-row"><div class="field-icon">#</div><div class="field-content"><div class="field-label">User ID</div><div class="field-value">${user.id}</div></div></div>
      <div class="field-row"><div class="field-icon">N</div><div class="field-content"><div class="field-label">Full Name</div><div class="field-value">${user.name}</div></div></div>
      <div class="field-row"><div class="field-icon">@</div><div class="field-content"><div class="field-label">Email Address</div><div class="field-value">${user.email}</div></div></div>
      <div class="field-row"><div class="field-icon">R</div><div class="field-content"><div class="field-label">Role</div><div class="field-value">${user.role}</div></div></div>
      <div class="field-row"><div class="field-icon">*</div><div class="field-content"><div class="field-label">Password</div><div class="field-value masked"><!-- &#9679;&#9679;&#9679;&#9679;&#9679;&#9679;&#9679;&#9679; --> ${user.password }</div></div></div>   
    </div>
    <div class="profile-actions">
      <a href="editprofile?userId=${user.id}" class="btn-update">&#9998; Update Profile</a>
