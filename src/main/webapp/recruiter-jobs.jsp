...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  /* ===== THEME VARS ===== */
  body.theme-purple { --bg:#faf5ff; --nav1:#4c1d95; --nav2:#7c3aed; --accent:#7c3aed; --accent-dark:#4c1d95; --accent-light:#ede9fe; --accent-border:#ddd6fe; --accent-text:#c4b5fd; --badge-bg:#7c3aed; --stat-num:#4c1d95; }
  body.theme-blue   { --bg:#f0f4ff; --nav1:#1e3a8a; --nav2:#3b82f6; --accent:#3b82f6; --accent-dark:#1e3a8a; --accent-light:#dbeafe; --accent-border:#bfdbfe; --accent-text:#93c5fd; --badge-bg:#3b82f6; --stat-num:#1e3a8a; }
  body.theme-green  { --bg:#f0fdf4; --nav1:#14532d; --nav2:#16a34a; --accent:#16a34a; --accent-dark:#14532d; --accent-light:#dcfce7; --accent-border:#bbf7d0; --accent-text:#86efac; --badge-bg:#16a34a; --stat-num:#14532d; }
  body.theme-rose   { --bg:#fff1f2; --nav1:#9f1239; --nav2:#e11d48; --accent:#e11d48; --accent-dark:#9f1239; --accent-light:#ffe4e6; --accent-border:#fecdd3; --accent-text:#fda4af; --badge-bg:#e11d48; --stat-num:#9f1239; }
  body.theme-dark   { --bg:#0f172a; --nav1:#0f172a; --nav2:#1e293b; --accent:#6366f1; --accent-dark:#4338ca; --accent-light:#1e1b4b; --accent-border:#312e81; --accent-text:#a5b4fc; --badge-bg:#6366f1; --stat-num:#a5b4fc; }
  ...
  .btn-nav {
    ...
    font-size: 13px; font-weight: 600; border: 1px solid rgba(255,255,255,0.25);
  ...
  .page-header {
    background: linear-gradient(135deg, var(--nav1), var(--nav2));
    color: #fff; padding: 36px 40px 50px;
  }
  ...
  .job-count-badge .count-num { font-size: 36px; font-weight: 900; color: #fff; }
  .job-count-badge .count-lbl { font-size: 12px; color: var(--accent-text); text-transform: uppercase; letter-spacing: 1px; margin-top: 2px; }

  .main { max-width: 1100px; margin: -24px auto 0; padding: 0 20px 60px; }

  .alert {
    border-radius: 10px; padding: 14px 20px; font-size: 14px; font-weight: 600;
    margin-bottom: 24px; display: flex; align-items: center; gap: 10px;
  }
  .alert-success { background: #dcfce7; border: 1px solid #86efac; border-left: 4px solid #16a34a; color: #15803d; }
  .alert-danger  { background: #fee2e2; border: 1px solid #fca5a5; border-left: 4px solid #dc2626; color: #b91c1c; }
  ...
  .card-header h3 { font-size: 16px; font-weight: 800; color: var(--accent-dark); }
  .btn-post {
  ...
  .btn-post:hover { transform: translateY(-2px); box-shadow: 0 6px 16px rgba(0,0,0,0.2); }
  ...
  tbody td { padding: 16px 18px; font-size: 14px; color: #1e293b; vertical-align: middle; }
  ...
  .job-desc { font-size: 12px; color: #64748b; margin-top: 3px; max-width: 280px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
  .location-tag { font-size: 12px; color: #64748b; }
  .salary-val { font-weight: 700; color: var(--accent-dark); }

  .action-btns { display: flex; gap: 8px; }
  .btn-edit {
    background: linear-gradient(135deg, #1e3a8a, #3b82f6);
    color: #fff; text-decoration: none; padding: 7px 16px;
  ...
  .empty-state p { color: #9ca3af; font-size: 14px; margin-bottom: 20px; }
  ...
  .modal-box .modal-icon { font-size: 44px; margin-bottom: 14px; }
  .modal-box h3 { font-size: 20px; font-weight: 800; color: #1e293b; margin-bottom: 8px; }
  .modal-box p { color: #64748b; font-size: 14px; margin-bottom: 28px; }
  .modal-actions { display: flex; gap: 12px; justify-content: center; }
  .btn-confirm-del {
    background: linear-gradient(135deg, #9f1239, #e11d48);
    color: #fff; padding: 11px 28px; border-radius: 9px;
    font-size: 14px; font-weight: 700; border: none; cursor: pointer;
    transition: transform 0.2s;
  }
  .btn-confirm-del:hover { transform: translateY(-2px); }
  .btn-cancel-del {
    background: #f3f4f6; color: #374151; padding: 11px 28px;
...
<div class="main">
  ...
  <% } else if("deleted".equals(msg)) { %>
    <div class="alert alert-danger">&#128465; Job deleted successfully.</div>
  <% } %>
            ...
            <tbody>
              <c:forEach var="job" items="${jobs}">
                <tr>
                  ...
                  <td><span class="salary-val">&#8377; ${job.salary} LPA</span></td>
                  <td>
                    <div class="action-btns">
                      <a href="editjob?jobId=${job.id}" class="btn-edit">&#9998; Edit</a>
                      <button class="btn-delete" onclick="confirmDelete(${job.id}, '${job.title}')">&#128465; Delete</button>
                    </div>
                  </td>
                </tr>
              </c:forEach>
  ...
  <a href="recruiter-dashboard.jsp" class="btn-back-link">&#8592; Back to Dashboard</a>
