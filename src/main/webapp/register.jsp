...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .nav-links a {
    color: #bfdbfe; text-decoration: none; margin-left: 20px;
  ...
  .form-header h1 { font-size: 26px; font-weight: 800; color: #1e293b; }
  ...
  /* ROLE SELECTOR */
  .role-label { font-size: 12px; font-weight: 600; color: #374151; margin-bottom: 10px; text-transform: uppercase; letter-spacing: 0.5px; }
  .role-cards { display: flex; gap: 14px; margin-bottom: 20px; }
  .role-card {
    flex: 1;
    border: 2px solid #e5e7eb;
    border-radius: 10px;
  ...
  .role-card.selected { border-color: #1d4ed8; background: #eff6ff; }
  .role-icon {
  ...
  .role-card-seeker .role-icon { background: #dbeafe; color: #1d4ed8; }
  .role-card-recruiter .role-icon { background: #ede9fe; color: #7c3aed; }
  .role-card h4 { font-size: 13px; font-weight: 700; color: #1e293b; margin-bottom: 4px; }
  .role-card p { font-size: 11px; color: #64748b; }
  ...
  .login-link a { color: #1d4ed8; font-weight: 700; text-decoration: none; }
  ...
  <div class="register-card">
    ...
    <form action="register" method="post">
      ...
      <div class="role-cards">
        <label class="role-card role-card-seeker" id="card-seeker" onclick="selectRole('JOB_SEEKER')">
          <input type="radio" name="role" value="JOB_SEEKER" id="role-seeker" checked>
          <div class="role-icon">J</div>
          <h4>Job Seeker</h4>
          <p>Find and apply to jobs</p>
        </label>
        <label class="role-card role-card-recruiter" id="card-recruiter" onclick="selectRole('RECRUITER')">
          <input type="radio" name="role" value="RECRUITER" id="role-recruiter">
...
function selectRole(val) {
  ...
  if(val === 'JOB_SEEKER') {
    document.getElementById('card-seeker').classList.add('selected');
    document.getElementById('role-seeker').checked = true;
  } else {
    document.getElementById('card-recruiter').classList.add('selected');
